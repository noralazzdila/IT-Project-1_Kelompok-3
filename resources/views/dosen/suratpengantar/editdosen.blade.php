...
@section('header-title', 'Edit Surat Pengantar')

@section('content')
<div class="card shadow-sm p-4">
    <h5 class="fw-bold mb-3">Formulir Edit Surat Pengantar</h5>
    
    <form action="{{ route('dosen.suratpengantar.updatedosen', $suratpengantar->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control @error('tanggal_pengajuan') is-invalid @enderror" id="tanggal_pengajuan" name="tanggal_pengajuan" value="{{ old('tanggal_pengajuan', \Carbon\Carbon::parse($suratpengantar->tanggal_pengajuan)->format('Y-m-d')) }}" required>
        ...
        <div class="mb-3">
            ...
            <input type="file" class="form-control @error('file_surat') is-invalid @enderror" id="file_surat" name="file_surat" accept=".pdf">
             @if($suratpengantar->file_surat)
                <small class="text-muted">File saat ini: <a href="{{ asset('storage/' . $suratpengantar->file_surat) }}" target="_blank">{{ $suratpengantar->file_surat }}</a></small>
            @endif
            @error('file_surat')<div class="invalid-feedback">{{ $message }}</div>@enderror
        </div>
        <div class="mb-3">
            <label for="catatan" class="form-label">Catatan</label>
            <textarea class="form-control @error('catatan') is-invalid @enderror" id="catatan" name="catatan" rows="4">{{ old('catatan', $suratpengantar->catatan) }}</textarea>
            @error('catatan')<div class="invalid-feedback">{{ $message }}</div>@enderror
        </div>
        <div class="mb-3">
            <label for="status" class="form-label">Status</label>
            <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                <option value="Revisi" {{ old('status', $suratpengantar->status) == 'Revisi' ? 'selected' : '' }}>Revisi</option>
                <option value="Diterima" {{ old('status', $suratpengantar->status) == 'Diterima' ? 'selected' : '' }}>Diterima</option>
                <option value="Ditolak" {{ old('status', $suratpengantar->status) == 'Ditolak' ? 'selected' : '' }}>Ditolak</option>
            </select>
