...
@section('header-title', 'Edit Data Proposal')

@section('content')
<div class="card shadow-sm p-4">
    <h5 class="fw-bold mb-3">Formulir Edit Proposal</h5>
    
    <form action="{{ route('dosen.proposal.updatedosen', $proposal->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control @error('tanggal_pengajuan') is-invalid @enderror" id="tanggal_pengajuan" name="tanggal_pengajuan" value="{{ old('tanggal_pengajuan', \Carbon\Carbon::parse($proposal->tanggal_pengajuan)->format('Y-m-d')) }}" required>
        ...
        <div class="mb-3">
            ...
            <input type="file" class="form-control @error('file_proposal') is-invalid @enderror" id="file_proposal" name="file_proposal" accept=".pdf">
            @if($proposal->file_proposal)
                <small class="text-muted">File saat ini: <a href="{{ asset('storage/' . $proposal->file_proposal) }}" target="_blank">{{ $proposal->file_proposal }}</a></small>
            @endif
            @error('file_proposal')<div class="invalid-feedback">{{ $message }}</div>@enderror
        </div>
        <div class="mb-3">
            <label for="catatan" class="form-label">Catatan</label>
            <textarea class="form-control @error('catatan') is-invalid @enderror" id="catatan" name="catatan" rows="4">{{ old('catatan', $proposal->catatan) }}</textarea>
        ...
        <div class="d-flex justify-content-end mt-3">
            <a href="{{ route('dosen.proposal.indexdosen') }}" class="btn btn-secondary me-2"><i class="fa fa-times"></i> Batal</a>
